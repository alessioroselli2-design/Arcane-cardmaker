...
// ========== CONFIG “TIPOGRAFICA” ==========
const CARD_W = 63;     // mm (tipo Magic 63×88)
const CARD_H = 88;     // mm
const GUTTER_X = 5;    // mm
const GUTTER_Y = 5;    // mm
const CROP_LEN = 3;    // mm
...
// Micro-offset retro (se in stampa vedi leggero disallineamento)
const BACK_OFFSET_X = 0;   // mm (prova 0.5 o 1 se serve)
...
function bindExportHandlers(){
  ...
  // --- PDF A4 3×3 (stessa carta) FRONTI ---
  ...
  // --- PDF A4 3×3 (stessa carta) FRONT+BACK ---
  $('#pdfA4Both')?.addEventListener('click',()=>{
    const PDF = jsPDForAlert(); if(!PDF) return;
    const doc = new PDF({ unit:'mm', format:'a4' });
    const { cells, pageW } = layoutA4Grid3x3();
    const fUrl = frontPNG(), bUrl = backPNG();
  ...
  // --- FOGLIO 3×3: aggiungi/svuota/esporta (carte diverse) ---
